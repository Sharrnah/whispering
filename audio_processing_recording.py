    ...
    def callback(self, in_data, frame_count, time_info, status):
        ...
        with torch.no_grad():
            ...
            if test_audio_chunk is not None:
                new_confidence, peak_amplitude = process_audio_chunk(test_audio_chunk, self.default_sample_rate, self.vad_model)
            ...
            if (clip_duration is not None and len(self.frames) > fps) or (
                ...
                self.push_to_talk_key) and len(self.frames) > 0) or (
                    use_speaker_diarization and self._new_speaker and self.diarization_model is not None and len(self.frames) > 0):
                ...
                if ((not vad_clip_test) or (vad_clip_test and full_audio_confidence >= confidence_threshold)) and len(
                    ...
                    wave_file_bytes = audio_tools.audio_bytes_to_wav(wavefiledata, channels=CHANNELS, sample_rate=SAMPLE_RATE)
                    if self.diarization_model is not None and use_speaker_diarization:
                        wave_file_bytes = self.diarization_model.diarize(wave_file_bytes, min_speakers=min_speakers, max_speakers=max_speakers)
            ...
            if self.should_stop_recording(new_confidence, confidence_threshold, peak_amplitude, energy, self.pause_time, pause,
                                     keyboard_key=self.push_to_talk_key):
