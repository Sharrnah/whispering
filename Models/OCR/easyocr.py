...
class EasyOcr(metaclass=SingletonMeta):
    ...
    def init_reader(self, languages):
        ...
        if self.reader is None or CURRENT_LANGUAGES != languages:
    ...
    def run_image_processing_from_image(self, image_src, src_languages):
        ...
        if self.reader is not None:
            try:
                result_data = self.reader.readtext(image, paragraph=True)
                if len(result_data) > 0:
                    for line in result_data:
                        # bbox to pixel value bbox
                        bbox_pixels = self.convert_bounding_box(line[0])
