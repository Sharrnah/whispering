...
class S3Token2Wav(S3Token2Mel):
    ...
    def forward(
        ...
        output_mels = super().forward(
            ...
            n_timesteps=n_timesteps, temperature=temperature, flow_cfg_scale=flow_cfg_scale, noised_mels=noised_mels,
        )
    ...
    def flow_inference(
        ...
        finalize: bool = False,
        speech_token_lens=None,
        n_timesteps: int|None = 10,
        temperature: float = 1.0,
        flow_cfg_scale: float = 0.7
    ):
        n_timesteps = n_timesteps or (2 if self.meanflow else 10)
        noise = None
        if self.meanflow:
        ...
        output_mels = super().forward(
            ...
            n_timesteps=n_timesteps, temperature=temperature, flow_cfg_scale=flow_cfg_scale, finalize=finalize, noised_mels=noise,
        )
        return output_mels
    ...
    def hift_inference(self, speech_feat, cache_source: torch.Tensor = None):
        ...
        if speech_feat is None or speech_feat.numel() == 0 or speech_feat.shape[-1] == 0:
            empty_wav = torch.zeros(1, 0, device=self.device, dtype=self.dtype)
