...
class Qwen3TTSModel:
    ...
    A HuggingFace-style wrapper for Qwen3 TTS models (CustomVoice/VoiceDesign/Base) that provides:
      - local from_pretrained() initialization via AutoModel/AutoProcessor
      - generation APIs for:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        """
        Load a Qwen3 TTS model and its processor from the verified application cache.
        ...
        This method:
          ...
          4) Loads `generation_config.json` from the model directory.
        ...
        if not os.path.isdir(pretrained_model_name_or_path):
            raise ValueError(
                "Qwen3-TTS must be loaded from the verified local application cache; "
                f"remote model identifiers are not allowed: {pretrained_model_name_or_path}"
            )
