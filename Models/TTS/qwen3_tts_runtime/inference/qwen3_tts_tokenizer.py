...
class Qwen3TTSTokenizer:
    ...
    - from_pretrained(): loads the speech tokenizer and feature extractor from a verified local directory.
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs) -> "Qwen3TTSTokenizer":
        """
        Initialize the tokenizer from the verified application cache.
        ...
        if not os.path.isdir(pretrained_model_name_or_path):
            raise ValueError(
                "Qwen3-TTS tokenizer weights must be loaded from the verified "
                f"local application cache; got {pretrained_model_name_or_path!r}."
            )
