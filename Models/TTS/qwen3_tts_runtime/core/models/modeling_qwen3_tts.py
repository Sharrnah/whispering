...
def _compute_default_rope_parameters(config, device=None, seq_len=None):
    """Transformers 5 equivalent of its former ``default`` RoPE entry."""
    del seq_len
...
def _restore_rotary_embedding_buffers(model):
    """Recreate RoPE buffers that are not part of the checkpoint state dict.

    Transformers 5 constructs models on the meta device during low-memory
    ``from_pretrained`` loading. These Qwen3-TTS buffers are non-persistent, so
    no checkpoint tensor replaces the meta allocation. Materializing that
    allocation can leave near-zero/uninitialized frequencies, effectively
    disabling positional encoding and producing repetitive, unintelligible
    speech. Recompute them after the model has reached its real device.
    """
    rotary_types = (Qwen3TTSTalkerRotaryEmbedding, Qwen3TTSRotaryEmbedding)
    for module in model.modules():
        if not isinstance(module, rotary_types):
            continue
...
class Qwen3TTSTalkerCodePredictorModel(Qwen3TTSPreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs,
        ...
        elif position_ids.shape[-1] != inputs_embeds.shape[1]:
            # Transformers 5 may retain the full position history while a
            # cached custom generation step supplies only the newest embed.
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Generation stage
            ...
            # Transformers 5 keeps the complete generated id history in
            # ``input_ids``.  This predictor consumes only the newest codebook
            # id at each cached decoding step (Transformers 4.x sliced it for
            # the model), so retaining the history would expand the sequence
            # and corrupt the multi-codebook prediction.
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if position_ids.shape[-1] != inputs_embeds.shape[1]:
            # Keep rotary embeddings aligned with the one-token query during
            # cached generation under Transformers 5.
            position_ids = position_ids[..., -inputs_embeds.shape[1]:]
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Generate
            ...
            # Transformers 5 passes the full generated id history here.  The
            # cached talker step needs only the newest codec id; using the
            # whole history also creates full-length position embeddings for
            # a one-token query and makes attention broadcast to an invalid
            # shape.
            if input_ids is not None and input_ids.shape[1] > 1:
                input_ids = input_ids[:, -1:]
            if position_ids is not None and position_ids.shape[-1] > 1:
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        speech_tokenizer_path = kwargs.pop("speech_tokenizer_path", None)
        if not os.path.isdir(pretrained_model_name_or_path):
            raise ValueError(
                "Qwen3-TTS weights must be loaded from a verified local directory; "
        ...
        if config is not None and requested_dtype is not None:
            # The talker and its code predictor use nested configs. Transformers
            # only annotates the outer config with the load dtype, which causes
            # a misleading FA2 warning even though all weights are loaded in
            # FP16/BF16. Keep the nested configs in sync before construction.
            config.dtype = requested_dtype
            config.talker_config.dtype = requested_dtype
            config.talker_config.code_predictor_config.dtype = requested_dtype
        ...
        _restore_rotary_embedding_buffers(model)
        speech_tokenizer_dir = speech_tokenizer_path or os.path.join(
            pretrained_model_name_or_path, "speech_tokenizer"
        )
        speech_tokenizer_config = os.path.join(speech_tokenizer_dir, "config.json")
        if not os.path.isfile(speech_tokenizer_config):
            raise FileNotFoundError(
                "The verified Qwen3-TTS 12 Hz speech tokenizer is missing: "
                f"{speech_tokenizer_config}"
            )
        ...
        if not os.path.isfile(generate_config_path):
            raise FileNotFoundError(f"Qwen3-TTS generation config is missing: {generate_config_path}")
    ...
    def generate(
        ...
        **kwargs,
        ...
        talker_kwargs = {
            ...
            # Codec codebooks are reconstructed from generation hidden states,
            # so these two values are mandatory.
            "output_hidden_states": True,
            "return_dict_in_generate": True,
        }
        # Upstream documents forwarding generic Transformers generation
        # options but accidentally treats this dict as an object. Keep the two
        # mandatory values above while forwarding options such as a stop
        # criterion and use_cache.
