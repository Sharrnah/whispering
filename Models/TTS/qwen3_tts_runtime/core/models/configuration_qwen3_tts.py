...
class Qwen3TTSSpeakerEncoderConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Transformers 4 populated several token/config defaults even when a
        # custom config did not call the base initializer. Transformers 5 only
        # exposes values that were explicitly initialized.
        super().__init__(
...
class Qwen3TTSTalkerCodePredictorConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Transformers 5 moved RoPE validation onto PretrainedConfig and uses
        # ``rope_parameters`` internally. Keep the upstream ``rope_scaling``
        # field for the Qwen model code while also populating the new form.
        self.rope_parameters = dict(self.rope_scaling or {})
