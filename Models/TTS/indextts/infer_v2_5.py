    ...
    def infer_generator(self, spk_audio_prompt, text, output_path, lang,
        ...
        for seg_idx, text_tokens in enumerate(segment_tokens):
            ...
            with torch.no_grad():
                ...
                if max_code_len == 0:
                    sampling_hint = (
                        " Try enabling sampling, which is the checkpoint's published default."
                        if not do_sample else ""
                    )
                    raise RuntimeError(
                        "IndexTTS generated no acoustic tokens for this text/reference pair."
                        + sampling_hint
                    )
