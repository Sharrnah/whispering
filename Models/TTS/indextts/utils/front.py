...
class TextNormalizer:
    ...
    def load(self):
        ...
        if self.zh_normalizer is not None and self.en_normalizer is not None:
            return
        # Use the same pinned, precompiled WeText/KaldiFST runtime on every
        # platform. The upstream Linux-only `tn` branch requires a separate
        # source grammar package that Whispering Tiger does not install.
