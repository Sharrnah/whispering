...
import transformers
from transformers import GenerationConfig, GenerationMixin, GPT2Config, LogitsProcessorList
...
try:
    ...
    # Transformers 5 removed the deprecated GPT-2 model-parallel helpers. The
    # application never enables this legacy multi-GPU path, but retain explicit
    # fallbacks so the inference class can still be constructed.
    def get_device_map(number_of_layers, devices):
        devices = list(devices)
        if not devices:
            return {"cpu": list(range(number_of_layers))}
        return {devices[0]: list(range(number_of_layers))}

    def assert_device_map(device_map, number_of_layers):
        mapped = sorted(layer for layers in device_map.values() for layer in layers)
        if mapped != list(range(number_of_layers)):
            raise ValueError("The device map must cover each GPT-2 layer exactly once.")
...
class GPT2InferenceModel(GPT2PreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, **kwargs):
        ...
        # Transformers 5 may pre-create a layer-backed DynamicCache whose
        # boolean value is true even though its sequence length is still zero.
        # That is the first generation step and must retain the full
        # conditioning/text prompt. Only slice to the last token after the
        # cache actually contains keys and values.
        cache_has_tokens = False
        if past_key_values is not None:
            if hasattr(past_key_values, "get_seq_length"):
                cache_has_tokens = past_key_values.get_seq_length() > 0
            else:
                cache_has_tokens = bool(past_key_values)
        # only last token for input_ids if a populated cache is defined
        if cache_has_tokens:
    ...
    def inference_speech(self, speech_condition, text_inputs, langs=None,
        ...
        if self.spk_cond_mode == "campplus":
            if campplus_embedding is not None:
                ...
                raise ValueError(
                    "Whispering Tiger requires the locally computed CAMPPlus "
                    "embedding and does not load a ModelScope speaker pipeline."
                )
            else:
